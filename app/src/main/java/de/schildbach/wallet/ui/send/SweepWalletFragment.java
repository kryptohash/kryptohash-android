...
import com.github.kryptohash.kryptohashj.crypto.BIP38PrivateKey;
...
public class SweepWalletFragment extends Fragment
{
	...
	private void restoreInstanceState(final Bundle savedInstanceState)
	{
		...
		{
			sentTransaction = application.getWallet().getTransaction((Shake320Hash) savedInstanceState.getSerializable("sent_transaction_hash"));
    ...
    private void maybeDecodeKey()
    {
        ...
        {
            final Ed25519Key key = ((DumpedPrivateKey) privateKeyToSweep).getKey();
            askConfirmSweep(key);
        }
        else if (privateKeyToSweep instanceof BIP38PrivateKey)
        {
            ...
            if (!password.isEmpty())
            {
                ...
                {
                    @Override
                    protected void onSuccess(@Nonnull Ed25519Key decryptedKey)
                    {
    ...
    private void askConfirmSweep(final Ed25519Key key)
	{
