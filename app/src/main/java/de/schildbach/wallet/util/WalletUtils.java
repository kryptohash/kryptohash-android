...
import com.github.kryptohash.kryptohashj.core.DumpedPrivateKey;
...
import com.github.kryptohash.kryptohashj.core.TransactionOutput;
...
public class WalletUtils
{
	...
	public static long longHash(@Nonnull final Sha3_256Hash hash)
	{
    ...
    public static long longHash320(@Nonnull final Shake320Hash hash)
    {
        final byte[] bytes = hash.getBytes();

        return (bytes[39] & 0xFFl) | ((bytes[38] & 0xFFl) << 8) | ((bytes[37] & 0xFFl) << 16) | ((bytes[36] & 0xFFl) << 24)
                | ((bytes[35] & 0xFFl) << 32) | ((bytes[34] & 0xFFl) << 40) | ((bytes[33] & 0xFFl) << 48) | ((bytes[32] & 0xFFl) << 56);
    }
	...
	public static void writeKeys(@Nonnull final Writer out, @Nonnull final List<Ed25519Key> keys) throws IOException
	{
		final DateFormat format = Iso8601Format.newDateTimeFormatT();

		out.write("# KEEP YOUR PRIVATE KEYS SAFE! Anyone who can read this can spend your KHCs.\n");
	...
	public static List<Ed25519Key> readKeys(@Nonnull final BufferedReader in) throws IOException
	{
		try
		{
			final DateFormat format = Iso8601Format.newDateTimeFormatT();

			final List<Ed25519Key> keys = new LinkedList<Ed25519Key>();
			...
			while (true)
			{
				...
				final Ed25519Key key = new DumpedPrivateKey(Constants.NETWORK_PARAMETERS, parts[0]).getKey();
