...
public class ExchangeRatesProvider extends ContentProvider
{
	...
	{
		public ExchangeRate(@Nonnull final com.github.kryptohash.kryptohashj.utils.ExchangeRate rate, final String source)
		{
			this.rate = rate;
			this.source = source;
		}

		public final com.github.kryptohash.kryptohashj.utils.ExchangeRate rate;
	...
	private static final long UPDATE_FREQ_MS = 30 * DateUtils.MINUTE_IN_MILLIS;
	private static final long RETRY_FREQ_MS = 5 * DateUtils.MINUTE_IN_MILLIS;
	...
	public Cursor query(final Uri uri, final String[] projection, final String selection, final String[] selectionArgs, final String sortOrder)
	{
			...
			if (newExchangeRates != null) {
				...
				if (exchangeRateToCache != null) {
					...
					if (khc_btc >= 0.0) {
						...
						exchangeRates.clear();
						lastUpdated += (UPDATE_FREQ_MS - RETRY_FREQ_MS);
					}
				}
			}
		}
		...
		if (selection == null) {
			for (final Map.Entry<String, ExchangeRate> entry : exchangeRates.entrySet())
			{
				...
				final com.github.kryptohash.kryptohashj.utils.ExchangeRate rate = exchangeRate.rate;
			...
			if (exchangeRate != null) {
				...
				final String currencyCode = exchangeRate.getCurrencyCode();
				cursor.newRow().add(currencyCode.hashCode()).add(currencyCode).add(khcRate.rate.coin.value).add(khcRate.rate.fiat.value).add(exchangeRate.source);
			}
		}
	...
	public static ExchangeRate getExchangeRate(@Nonnull final Cursor cursor)
	{
		...
		return new ExchangeRate(new com.github.kryptohash.kryptohashj.utils.ExchangeRate(rateCoin, rateFiat), source);
	}
	...
	private static Map<String, ExchangeRate> requestExchangeRates(final URL url, final String userAgent, final String source, final String... fields)
	{
		...
		try
		{
			...
			if (responseCode == HttpURLConnection.HTTP_OK)
			{
						...
						for (final String field : fields)
						{
							...
							if (rateStr != null)
							{
								try
								{
									...
									if (rate.signum() > 0)
									{
										rates.put(currencyCode, new ExchangeRate(new com.github.kryptohash.kryptohashj.utils.ExchangeRate(rate), source));
										break;
									}
								}
				...
				log.info("fetched exchange rates from {} ({}), {} chars, took {} ms", url, contentEncoding, length, System.currentTimeMillis() - start);
