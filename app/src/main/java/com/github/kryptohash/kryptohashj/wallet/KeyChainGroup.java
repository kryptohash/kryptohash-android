...
import com.github.kryptohash.kryptohashj.crypto.KeyCrypter;
...
/**
 * <p>A KeyChainGroup is used by the {@link com.github.kryptohash.kryptohashj.core.Wallet} and
...
public class KeyChainGroup implements KeyBag {
    ...
    private final EnumMap<KeyChain.KeyPurpose, DeterministicEd25519Key> currentKeys;

    // The map keys are the watching keys of the followed chains and values are the following chains
    private Multimap<DeterministicEd25519Key, DeterministicKeyChain> followingKeychains;
     ...
     */
    public KeyChainGroup(NetworkParameters params, DeterministicEd25519Key watchKey) {
     ...
     * This HAS to be an account key as returned by {@link DeterministicKeyChain#getWatchingKey()}.
     */
    public KeyChainGroup(NetworkParameters params, DeterministicEd25519Key watchKey, long creationTimeSecondsSecs) {
     ...
     */
    public KeyChainGroup(NetworkParameters params, DeterministicSeed seed, List<DeterministicEd25519Key> followingAccountKeys, int sigsRequiredToSpend) {
        this(params, seed);
     ...
     */
    public void addFollowingAccountKeys(List<DeterministicEd25519Key> followingAccountKeys, int sigsRequiredToSpend) {
        ...
        DeterministicEd25519Key accountKey = getActiveKeyChain().getWatchingKey();
        for (DeterministicEd25519Key key : followingAccountKeys) {
    ...
    private KeyChainGroup(NetworkParameters params, @Nullable BasicKeyChain basicKeyChain, List<DeterministicKeyChain> chains,
                          @Nullable EnumMap<KeyChain.KeyPurpose, DeterministicEd25519Key> currentKeys, Multimap<DeterministicEd25519Key,
        ...
        if (currentKeys == null)
            this.currentKeys = new EnumMap<KeyChain.KeyPurpose, DeterministicEd25519Key>(KeyChain.KeyPurpose.class);
        else
        ...
        if (!this.followingKeychains.isEmpty()) {
            DeterministicEd25519Key followedWatchKey = getActiveKeyChain().getWatchingKey();
            for (Map.Entry<KeyChain.KeyPurpose, DeterministicEd25519Key> entry : this.currentKeys.entrySet()) {
    ...
    private void maybeLookaheadScripts() {
        ...
        for (DeterministicKeyChain chain : chains) {
            if (isMarried(chain)) {
                ...
                for (DeterministicEd25519Key followedKey : chain.getLeafKeys()) {
    ...
    public DeterministicEd25519Key currentKey(KeyChain.KeyPurpose purpose) {
        ...
        DeterministicEd25519Key current = currentKeys.get(purpose);
        if (current == null) {
    ...
    public Address freshAddress(KeyChain.KeyPurpose purpose) {
        DeterministicKeyChain chain = getActiveKeyChain();
        if (isMarried(chain)) {
            List<Ed25519Key> marriedKeys = freshMarriedKeys(purpose, chain);
    ...
    private List<Ed25519Key> freshMarriedKeys(KeyChain.KeyPurpose purpose, DeterministicKeyChain followedKeyChain) {
        DeterministicEd25519Key followedKey = followedKeyChain.getKey(purpose);
        ImmutableList.Builder<Ed25519Key> keys = ImmutableList.<Ed25519Key>builder().add(followedKey);
        ...
        for (DeterministicKeyChain keyChain : keyChains) {
            DeterministicEd25519Key followingKey = keyChain.getKey(purpose);
    ...
    private List<Ed25519Key> getMarriedKeysWithFollowed(DeterministicEd25519Key followedKey, Collection<DeterministicKeyChain> followingChains) {
        ImmutableList.Builder<Ed25519Key> keys = ImmutableList.builder();
    ...
    public int importKeys(List<Ed25519Key> keys) {
    ...
    public int importKeysAndEncrypt(final List<Ed25519Key> keys, KeyParameter aesKey) {
        ...
        checkState(keyCrypter != null, "Not encrypted");
        LinkedList<Ed25519Key> encryptedKeys = Lists.newLinkedList();
    ...
    public void markP2SHAddressAsUsed(Address address) {
        ...
        for (Ed25519Key key : data.keys) {
            for (DeterministicKeyChain chain : chains) {
                DeterministicEd25519Key k = chain.findKeyFromPubKey(key.getPubKey());
                if (k == null) continue;
    ...
    private void maybeMarkCurrentKeyAsUsed(DeterministicEd25519Key key) {
        // It's OK for currentKeys to be empty here: it means we're a married wallet and the key may be a part of a
        // rotating chain.
        for (Map.Entry<KeyChain.KeyPurpose, DeterministicEd25519Key> entry : currentKeys.entrySet()) {
     ...
     */
    public boolean removeImportedKey(Ed25519Key key) {
        checkNotNull(key);
        checkArgument(!(key instanceof DeterministicEd25519Key));
    ...
    public boolean isMarried(DeterministicKeyChain keychain) {
        DeterministicEd25519Key watchingKey = keychain.getWatchingKey();
     ...
     *
     * @throws com.github.kryptohash.kryptohashj.crypto.KeyCrypterException Thrown if the wallet encryption fails for some reason,
     ...
     * KeyCrypter to use is {@link com.github.kryptohash.kryptohashj.crypto.KeyCrypterScrypt}.
     *
     * @throws com.github.kryptohash.kryptohashj.crypto.KeyCrypterException Thrown if the wallet decryption fails for some reason, leaving the group unchanged.
     */
    ...
    private Script makeP2SHOutputScript(List<Ed25519Key> marriedKeys) {
        return ScriptBuilder.createP2SHOutputScript(makeRedeemScript(marriedKeys));
    }

    private Script makeP2SHOutputScript(DeterministicEd25519Key followedKey, DeterministicEd25519Key followedAccountKey) {
        return ScriptBuilder.createP2SHOutputScript(getRedeemData(followedKey, followedAccountKey).redeemScript);
    }

    private RedeemData getRedeemData(DeterministicEd25519Key followedKey, DeterministicEd25519Key followedAccountKey) {
        Collection<DeterministicKeyChain> followingChains = followingKeychains.get(followedAccountKey);
        List<Ed25519Key> marriedKeys = getMarriedKeysWithFollowed(followedKey, followingChains);
        return RedeemData.of(marriedKeys, makeRedeemScript(marriedKeys));
    }

    private Script makeRedeemScript(List<Ed25519Key> marriedKeys) {
    ...
    public static KeyChainGroup fromProtobufUnencrypted(NetworkParameters params, List<Protos.Key> keys, int sigsRequiredToSpend) throws UnreadableWalletException {
        ...
        Multimap<DeterministicEd25519Key, DeterministicKeyChain> followingKeychains = extractFollowingKeychains(chains);
    ...
    public DeterministicKeyChain upgradeToDeterministic(long keyRotationTimeSecs, @Nullable KeyParameter aesKey) throws DeterministicUpgradeRequiresPassword, AllRandomKeysRotating {
        ...
        Ed25519Key keyToUse = basic.findOldestKeyAfter(keyRotationTimeSecs - 1);
        if (keyToUse == null)
    ...
    private static EnumMap<KeyChain.KeyPurpose, DeterministicEd25519Key> createCurrentKeysMap(List<DeterministicKeyChain> chains) {
        DeterministicKeyChain activeChain = chains.get(chains.size() - 1);

        EnumMap<KeyChain.KeyPurpose, DeterministicEd25519Key> currentKeys = new EnumMap<KeyChain.KeyPurpose, DeterministicEd25519Key>(KeyChain.KeyPurpose.class);
        ...
        if (activeChain.getIssuedExternalKeys() > 0) {
            DeterministicEd25519Key currentExternalKey = activeChain.getKeyByPath(
        ...
        if (activeChain.getIssuedInternalKeys() > 0) {
            DeterministicEd25519Key currentInternalKey = activeChain.getKeyByPath(
    ...
    private static Multimap<DeterministicEd25519Key, DeterministicKeyChain> extractFollowingKeychains(List<DeterministicKeyChain> chains) {
    ...
    public String toString(boolean includePrivateKeys) {
        ...
        if (basic != null) {
            List<Ed25519Key> keys = basic.getKeys();
            Collections.sort(keys, Ed25519Key.AGE_COMPARATOR);
            for (Ed25519Key key : keys)
        ...
        for (DeterministicKeyChain chain : chains) {
            ...
            final DeterministicEd25519Key watchingKey = chain.getWatchingKey();
            ...
            if (isMarried(chain)) {
                ...
                for (Ed25519Key key : chain.getKeys(false))
    ...
    private void formatKeyWithAddress(boolean includePrivateKeys, Ed25519Key key, StringBuilder builder) {
        ...
        if (key instanceof DeterministicEd25519Key) {
            builder.append("  (");
            builder.append((((DeterministicEd25519Key) key).getPathAsString()));
            builder.append(")");
        }
        builder.append("\n");
