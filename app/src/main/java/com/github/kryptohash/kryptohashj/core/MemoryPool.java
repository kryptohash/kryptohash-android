...
public class MemoryPool {
    ...
    private LinkedHashMap<Shake320Hash, CEntry> memoryPool;
    ...
    public MemoryPool(final int size) {
        memoryPool = new LinkedHashMap<Shake320Hash, CEntry>() {
            @Override
            protected boolean removeEldestEntry(Map.Entry<Shake320Hash, CEntry> entry) {
     ...
     */
    public int numBroadcastPeers(Shake320Hash txHash) {
        lock.lock();
        try {
            cleanPool();
            CEntry entry = memoryPool.get(txHash);
            if (entry == null) {
                // No such TX known.
                return 0;
    ...
    public Transaction intern(Transaction tx) {
        ...
        try {
            cleanPool();
            CEntry entry = memoryPool.get(tx.getHash());
            if (entry != null) {
     ...
     */
    public void seen(Shake320Hash hash, PeerAddress byPeer) {
        lock.lock();
        try {
            cleanPool();
            CEntry entry = memoryPool.get(hash);
            if (entry != null) {
     ...
     */
    @Nullable
    public Transaction get(Shake320Hash hash) {
        lock.lock();
        try {
            CEntry entry = memoryPool.get(hash);
     ...
     */
    public boolean maybeWasSeen(Shake320Hash hash) {
        lock.lock();
        try {
            CEntry entry = memoryPool.get(hash);
            return entry != null;
