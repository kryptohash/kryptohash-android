...
public class Utils {
    ...
    public static byte[] doubleDigest(byte[] input) {
        return KryptohashUtils.doubleSha3_256(input, 0, input.length).getBytes();
    }

    /**
     * Calculates the SHA3-256 hash of the given byte range, and then hashes the resulting hash again.
     * The resulting hash is in big endian form.
     */
    public static byte[] doubleDigest(byte[] input, int offset, int length) {
        return KryptohashUtils.doubleSha3_256(input, offset, length).getBytes();
    }

    /**
     * Calculates the SHAKE320 hash of the given byte range.
     * The resulting hash is in big endian form.
     */
    public static byte[] singleDigest(byte[] input) {
        return KryptohashUtils.Shake320(input).getBytes();
    }

    public static byte[] singleDigestTwoBuffers(byte[] input1, byte[] input2) {
        return KryptohashUtils.Shake320(input1, input2).getBytes();
    }

    public static byte[] singleDigest(byte[] input, int offset, int length) {
        return KryptohashUtils.Shake320(input, offset, length).getBytes();
    }

    /**
     * Calculates SHA3_256(SHA3_256(byte range 1 + byte range 2)).
     */
    public static byte[] doubleDigestTwoBuffers(byte[] input1, int offset1, int length1,
                                                byte[] input2, int offset2, int length2) {
        return KryptohashUtils.doubleSha3_256TwoBuffers(input1, offset1, length1, input2, offset2, length2).getBytes();
    }
