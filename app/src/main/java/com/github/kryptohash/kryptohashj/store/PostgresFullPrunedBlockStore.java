 ...
 * so you can use {@link #calculateBalanceForAddress(com.github.kryptohash.kryptohashj.core.Address)} to quickly look up
...
public class PostgresFullPrunedBlockStore implements FullPrunedBlockStore {
    ...
    private Shake320Hash chainHeadHash;
    private StoredBlock chainHeadBlock;
    private Shake320Hash verifiedChainHeadHash;
    ...
    private void initFromDatabase() throws SQLException, BlockStoreException {
        ...
        Shake320Hash hash = new Shake320Hash(rs.getBytes(1));
        rs.close();
        ...
        hash = new Shake320Hash(rs.getBytes(1));
        rs.close();
        s.close();
    ...
    public StoredBlock get(Shake320Hash hash, boolean wasUndoableOnly) throws BlockStoreException {
    ...
    @Override
    public StoredBlock get(Shake320Hash hash) throws BlockStoreException {
        return get(hash, false);
    }

    @Override
    public StoredBlock getOnceUndoableStoredBlock(Shake320Hash hash) throws BlockStoreException {
        return get(hash, true);
    }

    @Override
    public StoredUndoableBlock getUndoBlock(Shake320Hash hash) throws BlockStoreException {
        maybeConnect();
        PreparedStatement s = null;
        try {
            s = conn.get()
    ...
    public void setChainHead(StoredBlock chainHead) throws BlockStoreException {
        Shake320Hash hash = chainHead.getHeader().getHash();
        this.chainHeadHash = hash;
    ...
    @Override
    public StoredTransactionOutput getTransactionOutput(Shake320Hash hash, long index) throws BlockStoreException {
        maybeConnect();
        PreparedStatement s = null;
        try {
            s = conn.get()
    ...
    @Override
    public boolean hasUnspentOutputs(Shake320Hash hash, int numOutputs) throws BlockStoreException {
        maybeConnect();
        PreparedStatement s = null;
        try {
            s = conn.get()
