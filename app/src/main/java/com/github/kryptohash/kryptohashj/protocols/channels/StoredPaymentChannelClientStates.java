...
public class StoredPaymentChannelClientStates implements WalletExtension {
    ...
    @GuardedBy("lock") @VisibleForTesting final HashMultimap<Sha3_256Hash, StoredClientChannel> mapChannels = HashMultimap.create();
    ...
    public Coin getBalanceForServer(Sha3_256Hash id) {
     ...
     * channels found.
     */
    public long getSecondsUntilExpiry(Sha3_256Hash id) {
        lock.lock();
        try {
     ...
     */
    @Nullable
    StoredClientChannel getUsableChannelForServerID(Sha3_256Hash id) {
        lock.lock();
        try {
            Set<StoredClientChannel> setChannels = mapChannels.get(id);
            for (StoredClientChannel channel : setChannels) {
     ...
     */
    @Nullable
    StoredClientChannel getChannel(Sha3_256Hash id, Shake320Hash contractHash) {
        lock.lock();
        try {
            Set<StoredClientChannel> setChannels = mapChannels.get(id);
            for (StoredClientChannel channel : setChannels) {
    ...
    public void deserializeWalletExtension(Wallet containingWallet, byte[] data) throws Exception {
        ...
        try {
            ...
            for (ClientState.StoredClientPaymentChannel storedState : states.getChannelsList()) {
                ...
                StoredClientChannel channel = new StoredClientChannel(new Sha3_256Hash(storedState.getId().toByteArray()),
                        new Transaction(params, storedState.getContractTransaction().toByteArray()),
                        refundTransaction,
                        Ed25519Key.fromSeed(storedState.getMyKey().toByteArray()),
                        Coin.valueOf(storedState.getValueToMe()),
                        Coin.valueOf(storedState.getRefundFees()), false);
                if (storedState.hasCloseTransactionHash()) {
                    Shake320Hash closeTxHash = new Shake320Hash(storedState.getCloseTransactionHash().toByteArray());
...
class StoredClientChannel {
    ...
    StoredClientChannel(Sha3_256Hash id, Transaction contract, Transaction refund, Ed25519Key myKey, Coin valueToMe,
