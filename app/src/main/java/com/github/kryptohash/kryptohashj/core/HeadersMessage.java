...
public class HeadersMessage extends Message {
    ...
    protected void parseLite() throws ProtocolException {
        if (length == UNKNOWN_LENGTH) {
            ...
            // Each header has 120 bytes and one more byte for transactions number which is 00.
            length = 121 * (int)numHeaders;
        }
    }
    ...
    void parse() throws ProtocolException {
        ...
        for (int i = 0; i < numHeaders; ++i) {
            // Read 120 bytes of the header and two more bytes for the transaction list, which is always a 00 00 because the
            // transaction list is empty.
            byte[] blockHeader = readBytes(122);
            if (blockHeader[120] != 0)
                throw new ProtocolException("Block header does not end with a null byte");
            Block newBlockHeader = new Block(this.params, blockHeader, true, true, 121);
