...
package com.github.kryptohash.kryptohashj.script;
...
import static com.github.kryptohash.kryptohashj.script.ScriptOpCodes.*;
...
public class Script {
    ...
    public Address getToAddress(NetworkParameters params, boolean forcePayToPubKey) throws ScriptException {
        if (isSentToAddress())
            ...
            return Ed25519Key.fromPublicOnly(getPubKey()).toAddress(params);
        else
    ...
    public static byte[] createMultiSigOutputScript(int threshold, List<Ed25519Key> pubkeys) {
     ...
     */
    public Script createEmptyInputScript(@Nullable Ed25519Key key, @Nullable Script redeemScript) {
        if (isSentToAddress()) {
     ...
     * a P2SH scriptSig.
     */
    public int getSigInsertionIndex(Sha3_256Hash hash, Ed25519Key signingKey) {
    ...
    private int findKeyInRedeem(Ed25519Key key) {
    ...
    private int findSigInRedeem(byte[] signatureBytes, Sha3_256Hash hash) {
        checkArgument(chunks.get(0).isOpCode()); // P2SH scriptSig
        int numKeys = Script.decodeFromOpN(chunks.get(chunks.size() - 2).opcode);
        TxEd25519Signature signature = TxEd25519Signature.decodeFromKryptohash(signatureBytes);
        for (int i = 0 ; i < numKeys ; i++) {
            if (Ed25519Key.fromPublicOnly(chunks.get(i + 1).data).verify(hash, signature)) {
                return i;
            }
        }
    ...
    /**
     * Returns number of bytes required to spend this script. It accepts optional Ed25519Key and redeemScript that may
     * be required for certain types of script to estimate target size.
     */
    public int getNumberOfBytesRequiredToSpend(@Nullable Ed25519Key pubKey, @Nullable Script redeemScript) {
        if (isPayToScriptHash()) {
     ...
     * {@link com.github.kryptohash.kryptohashj.core.TransactionInput#verify(com.github.kryptohash.kryptohashj.core.TransactionOutput)} or
     * {@link com.github.kryptohash.kryptohashj.script.Script#correctlySpends(com.github.kryptohash.kryptohashj.core.Transaction, long, Script)}. This method
        ...
        for (ScriptChunk chunk : script.chunks) {
            ...
            if (!chunk.isOpCode()) {
                ...
                case OP_SHAKE160:
                    ...
                    stack.add(KryptohashUtils.Shake160(stack.pollLast()).getBytes());
                    break;
                case OP_HASH256:
                    if (stack.size() < 1)
                        throw new ScriptException("Attempted OP_SHA256 on an empty stack");
                ...
                case OP_SHA3_224:
                    if (stack.size() < 1)
                        throw new ScriptException("Attempted OP_SHA3_224 on an empty stack");
                    stack.add(KryptohashUtils.Sha3_224(stack.pollLast()).getBytes());
                    break;
                case OP_SHA3_256:
                    if (stack.size() < 1)
                        throw new ScriptException("Attempted OP_SHA3_256 on an empty stack");
                    stack.add(KryptohashUtils.Sha3_256(stack.pollLast()).getBytes());
                    break;
                case OP_SHAKE320:
                    if (stack.size() < 1)
                        throw new ScriptException("Attempted OP_SHAKE320 on an empty stack");
                    stack.add(KryptohashUtils.Shake320(stack.pollLast()).getBytes());
                    break;
                case OP_NOP5:
                case OP_NOP6:
                case OP_NOP7:
    ...
    private static void executeCheckSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack, int lastCodeSepLocation, int opcode) throws ScriptException {
        ...
        try {
            TxEd25519Signature sig = TxEd25519Signature.decodeFromKryptohash(sigBytes);
            Sha3_256Hash hash = txContainingThis.hashForSignature(index, connectedScript, (byte) sig.sighashFlags);
            sigValid = Ed25519Key.verify(hash.getBytes(), sig, pubKey);
        } catch (Exception e1) {
            ...
            if (!e1.getMessage().contains("Zero length signature"))
    ...
    private static int executeMultiSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack, int opCount, int lastCodeSepLocation, int opcode, boolean enforceNullDummy) throws ScriptException {
        if (stack.size() < 2)
        ...
        while (sigs.size() > 0) {
            ...
            try {
                TxEd25519Signature sig = TxEd25519Signature.decodeFromKryptohash(sigs.getFirst());
                Sha3_256Hash hash = txContainingThis.hashForSignature(index, connectedScript, (byte) sig.sighashFlags);
                if (Ed25519Key.verify(hash.getBytes(), sig, pubKey))
                    sigs.pollFirst();
            } catch (Exception e) {
                // There is (at least) one exception that could be hit here (EOFException, if the sig is too short)
                // Because I can't verify there aren't more, we use a very generic Exception catch
