 ...
 * {@link com.github.kryptohash.kryptohashj.crypto.DeterministicHierarchy}, to derive all the keys in the keychain from a master seed.
 ...
 * written down on paper or, when represented using a BIP 39 {@link com.github.kryptohash.kryptohashj.crypto.MnemonicCode},
 ...
 * {@link com.github.kryptohash.kryptohashj.crypto.DeterministicEd25519Key#serializePubB58()}. The resulting "xpub..." string encodes
 * sufficient information about the account key to create a watching chain via
 * {@link com.github.kryptohash.kryptohashj.crypto.DeterministicEd25519Key#deserializeB58(com.github.kryptohash.kryptohashj.crypto.DeterministicEd25519Key, String)}
 * (with null as the first parameter) and then
 * {@link DeterministicKeyChain#DeterministicKeyChain(com.github.kryptohash.kryptohashj.crypto.DeterministicEd25519Key)}.</p>
 *
 * <p>This class builds on {@link com.github.kryptohash.kryptohashj.crypto.DeterministicHierarchy} and
 * {@link com.github.kryptohash.kryptohashj.crypto.DeterministicEd25519Key} by adding support for serialization to and from protobufs,
...
public class DeterministicKeyChain implements EncryptableKeyChain {
    ...
    @Nullable private DeterministicEd25519Key rootKey;
    ...
    private DeterministicEd25519Key externalKey, internalKey;
     ...
     */
    public DeterministicKeyChain(DeterministicEd25519Key watchingKey, long creationTimeSeconds) {
    ...
    public DeterministicKeyChain(DeterministicEd25519Key watchingKey) {
     ...
     */
    private DeterministicKeyChain(DeterministicEd25519Key watchKey, boolean isFollowing) {
     ...
     */
    public static DeterministicKeyChain watchAndFollow(DeterministicEd25519Key watchKey) {
     ...
     */
    public static DeterministicKeyChain watch(DeterministicEd25519Key accountKey) {
     ...
     */
    public static DeterministicKeyChain watch(DeterministicEd25519Key accountKey, long seedCreationTimeSecs) {
    ...
    DeterministicKeyChain(DeterministicSeed seed, @Nullable KeyCrypter crypter) {
        ...
        if (!seed.isEncrypted()) {
            ...
            if (!rootKey.hasSeed())
                throw new RuntimeException("Could not create Master Key");
    ...
    private DeterministicKeyChain(KeyCrypter crypter, KeyParameter aesKey, DeterministicKeyChain chain) {
        ...
        DeterministicEd25519Key account = encryptNonLeaf(aesKey, chain, rootKey, ACCOUNT_ZERO_PATH);
        ...
        for (Ed25519Key eckey : chain.basicKeyChain.getKeys()) {
            DeterministicEd25519Key key = (DeterministicEd25519Key) eckey;
            if (key.getPath().size() != 3) continue; // Not a leaf key.
            DeterministicEd25519Key parent = hierarchy.get(checkNotNull(key.getParent()).getPath(), false, false);
            // Clone the key to the new encrypted hierarchy.
            key = new DeterministicEd25519Key(key.getPubOnly(), parent);
            hierarchy.putKey(key);
            basicKeyChain.importKey(key);
        }
    }

    private DeterministicEd25519Key encryptNonLeaf(KeyParameter aesKey, DeterministicKeyChain chain,
                                                   DeterministicEd25519Key parent, ImmutableList<ChildNumber> path) {
        DeterministicEd25519Key key = chain.hierarchy.get(path, false, false);
    ...
    private void initializeHierarchyUnencrypted(DeterministicEd25519Key baseKey) {
    ...
    @Override
    public DeterministicEd25519Key getKey(KeyPurpose purpose) {
        return getKeys(purpose, 1).get(0);
    }

    /** Returns freshly derived key/s that have not been returned by this method before. */
    @Override
    public List<DeterministicEd25519Key> getKeys(KeyPurpose purpose, int numberOfKeys) {
        checkArgument(numberOfKeys > 0);
        lock.lock();
        try {
            DeterministicEd25519Key parentKey;
            int index;
            switch (purpose) {
            ...
            List<DeterministicEd25519Key> lookahead = maybeLookAhead(parentKey, index, 0, 0);
            basicKeyChain.importKeys(lookahead);
            List<DeterministicEd25519Key> keys = new ArrayList<DeterministicEd25519Key>(numberOfKeys);
            for (int i = 0; i < numberOfKeys; i++) {
                ImmutableList<ChildNumber> path = HDUtils.append(parentKey.getPath(), new ChildNumber(index - numberOfKeys + i, false));
                DeterministicEd25519Key k = hierarchy.get(path, false, false);
    ...
    private void addToBasicChain(DeterministicEd25519Key key) {
     ...
     */
    public DeterministicEd25519Key markKeyAsUsed(DeterministicEd25519Key k) {
    ...
    public DeterministicEd25519Key findKeyFromPubHash(byte[] pubkeyHash) {
        lock.lock();
        try {
            return (DeterministicEd25519Key) basicKeyChain.findKeyFromPubHash(pubkeyHash);
        } finally {
            lock.unlock();
        }
    }

    public DeterministicEd25519Key findKeyFromPubKey(byte[] pubkey) {
        lock.lock();
        try {
            return (DeterministicEd25519Key) basicKeyChain.findKeyFromPubKey(pubkey);
        } finally {
            lock.unlock();
        }
    }
     ...
     * See {@link DeterministicKeyChain#markKeyAsUsed(DeterministicEd25519Key)} for more info on this.
     */
    @Nullable
    public DeterministicEd25519Key markPubHashAsUsed(byte[] pubkeyHash) {
        lock.lock();
        try {
            DeterministicEd25519Key k = (DeterministicEd25519Key) basicKeyChain.findKeyFromPubHash(pubkeyHash);
            if (k != null)
                markKeyAsUsed(k);
            return k;
        } finally {
     ...
     */
    @Nullable
    public DeterministicEd25519Key markPubKeyAsUsed(byte[] pubkey) {
        lock.lock();
        try {
            DeterministicEd25519Key k = (DeterministicEd25519Key) basicKeyChain.findKeyFromPubKey(pubkey);
            if (k != null)
                markKeyAsUsed(k);
            return k;
        } finally {
    ...
    protected DeterministicEd25519Key getKeyByPath(ChildNumber... path) {
        return getKeyByPath(ImmutableList.<ChildNumber>copyOf(path));
    }

    /** Returns the deterministic key for the given absolute path in the hierarchy. */
    protected DeterministicEd25519Key getKeyByPath(List<ChildNumber> path) {
    ...
    public DeterministicEd25519Key getKeyByPath(List<ChildNumber> path, boolean create) {
     ...
     * The returned key can be serialized and then passed into {@link #watch(com.github.kryptohash.kryptohashj.crypto.DeterministicEd25519Key)}
    ...
    public List<Protos.Key> serializeToProtobuf() {
        ...
        try {
            ...
            Map<Ed25519Key, Protos.Key.Builder> keys = basicKeyChain.serializeToEditableProtobufs();
            for (Map.Entry<Ed25519Key, Protos.Key.Builder> entry : keys.entrySet()) {
                DeterministicEd25519Key key = (DeterministicEd25519Key) entry.getKey();
                Protos.Key.Builder proto = entry.getValue();
                proto.setType(Protos.Key.Type.DETERMINISTIC_KEY);
                final Protos.DeterministicEd25519Key.Builder detKey = proto.getDeterministicKeyBuilder();
    ...
    public static List<DeterministicKeyChain> fromProtobuf(List<Protos.Key> keys, @Nullable KeyCrypter crypter) throws UnreadableWalletException {
        ...
        for (Protos.Key key : keys) {
            ...
            if (t == Protos.Key.Type.DETERMINISTIC_MNEMONIC) {
                ...
                BigInteger pubkey = new BigInteger(1, key.getPublicKey().toByteArray());
                ...
                if (chain == null) {
                    if (seed == null) {
                        DeterministicEd25519Key accountKey = new DeterministicEd25519Key(immutablePath, chainCode, pubkey, null, null);
                ...
                if (key.hasSecretBytes()) {
                    ...
                    if (key.hasEncryptedData()) {
                        ...
                        detkey = new DeterministicEd25519Key(immutablePath, chainCode, crypter, pubkey, data, parent);
                    } else {
                        ...
                        detkey = new DeterministicEd25519Key(immutablePath, chainCode, pubkey, null, parent);
                    }
                }
    ...
    public DeterministicKeyChain toDecrypted(KeyParameter aesKey) {
        ...
        for (Ed25519Key eckey : basicKeyChain.getKeys()) {
            DeterministicEd25519Key key = (DeterministicEd25519Key) eckey;
            if (key.getPath().size() != 3) continue; // Not a leaf key.
            checkState(key.isEncrypted());
            DeterministicEd25519Key parent = chain.hierarchy.get(checkNotNull(key.getParent()).getPath(), false, false);
    ...
    public void maybeLookAhead() {
        lock.lock();
        try {
            List<DeterministicEd25519Key> keys = maybeLookAhead(externalKey, issuedExternalKeys);
    ...
    private List<DeterministicEd25519Key> maybeLookAhead(DeterministicEd25519Key parent, int issued) {
     ...
     */
    private List<DeterministicEd25519Key> maybeLookAhead(DeterministicEd25519Key parent, int issued, int lookaheadSize, int lookaheadThreshold) {
        checkState(lock.isHeldByCurrentThread());
        ...
        if (needed <= lookaheadThreshold)
            return new ArrayList<DeterministicEd25519Key>();
        ...
        List<DeterministicEd25519Key> result  = new ArrayList<DeterministicEd25519Key>(needed);
        long now = System.currentTimeMillis();
        int nextChild = numChildren;
        for (int i = 0; i < needed; i++) {
            DeterministicEd25519Key key = HDKeyDerivation.deriveThisOrNextChildKey(parent, nextChild);
            key = key.getPrivPub();
            hierarchy.putKey(key);
            result.add(key);
    ...
    /* package */ List<Ed25519Key> getKeys(boolean includeLookahead) {
        List<Ed25519Key> keys = basicKeyChain.getKeys();
        if (!includeLookahead) {
            int treeSize = internalKey.getPath().size();
            List<Ed25519Key> issuedKeys = new LinkedList<Ed25519Key>();
            for (Ed25519Key key : keys) {
                DeterministicEd25519Key detkey = (DeterministicEd25519Key) key;
                DeterministicEd25519Key parent = detkey.getParent();
     ...
     */
    public List<DeterministicEd25519Key> getLeafKeys() {
        ImmutableList.Builder<DeterministicEd25519Key> keys = ImmutableList.builder();
        for (Ed25519Key key : getKeys(true)) {
            DeterministicEd25519Key dKey = (DeterministicEd25519Key) key;
