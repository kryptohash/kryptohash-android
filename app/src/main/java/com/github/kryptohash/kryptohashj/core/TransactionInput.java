...
import com.github.kryptohash.kryptohashj.wallet.DefaultRiskAnalysis;
...
public class TransactionInput extends ChildMessage implements Serializable {
    ...
    public static final long GENESIS = 0xFFFFFFFEL;
    ...
    public TransactionInput(NetworkParameters params, byte[] scriptBytes, @Nullable Transaction parentTransaction) {
        this(params, parentTransaction, scriptBytes, new TransactionOutPoint(params, GENESIS, (Transaction) null));
    }
    ...
    public TransactionInput(NetworkParameters params, @Nullable Transaction parentTransaction, byte[] scriptBytes,
        ...
        length = 48 + (scriptBytes == null ? 1 : VarInt.sizeOf(scriptBytes.length) + scriptBytes.length);
    }
    ...
    public boolean isCoinBase() {
        maybeParse();
        return outpoint.getHash().equals(Shake320Hash.ZERO_HASH) &&
                (outpoint.getIndex() & 0xFFFFFFFFL) == 0xFFFFFFFFL;  // -1 but all is serialized to the wire as unsigned int.
    }

    /**
     * Genesis is a special case.
     */
    public boolean isGenesis() {
        maybeParse();
        return outpoint.getHash().equals(Shake320Hash.ZERO_HASH) &&
                (outpoint.getIndex() & 0xFFFFFFFFL) == 0xFFFFFFFEL;  // -2 but all is serialized to the wire as unsigned int.
    }
    ...
    void setScriptBytes(byte[] scriptBytes) {
        ...
        this.scriptBytes = scriptBytes;
        // 48 = previous_outpoint (44) + sequence (4)
        int newLength = 48 + (scriptBytes == null ? 1 : VarInt.sizeOf(scriptBytes.length) + scriptBytes.length);
     ...
     */
    @Nullable
    TransactionOutput getConnectedOutput(Map<Shake320Hash, Transaction> transactions) {
     ...
     * @see TransactionOutPoint#getConnectedRedeemData(com.github.kryptohash.kryptohashj.wallet.KeyBag)
     */
    @Nullable
     ...
     */
    public ConnectionResult connect(Map<Shake320Hash, Transaction> transactions, ConnectMode mode) {
        Transaction tx = transactions.get(outpoint.getHash());
        if (tx == null) {
