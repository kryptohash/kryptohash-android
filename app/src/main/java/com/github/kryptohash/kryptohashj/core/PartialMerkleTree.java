 ...
 *  - uint320[]  hashes in depth-first order (<= 32*N bytes)
...
public class PartialMerkleTree extends Message {
    ...
    private List<Shake320Hash> hashes;
     ...
     */
    public PartialMerkleTree(NetworkParameters params, byte[] bits, List<Shake320Hash> hashes, int origTxCount) {
        super(params);
     ...
     */
    public static PartialMerkleTree buildFromLeaves(NetworkParameters params, byte[] includeBits, List<Shake320Hash> allLeafHashes) {
        // Calculate height of the tree.
        int height = 0;
        while (getTreeWidth(allLeafHashes.size(), height) > 1)
            height++;
        List<Boolean> bitList = new ArrayList<Boolean>();
        List<Shake320Hash> hashes = new ArrayList<Shake320Hash>();
    ...
    void parse() throws ProtocolException {
        ...
        hashes = new ArrayList<Shake320Hash>(nHashes);
    ...
    private static void traverseAndBuild(int height, int pos, List<Shake320Hash> allLeafHashes, byte[] includeBits,
                                         List<Boolean> matchedChildBits, List<Shake320Hash> resultHashes) {
    ...
    private static Shake320Hash calcHash(int height, int pos, List<Shake320Hash> hashes) {
        if (height == 0) {
        ...
        int h = height - 1;
        int p = pos * 2;
        Shake320Hash left = calcHash(h, p, hashes);
    ...
    private Shake320Hash recursiveExtractHashes(int height, int pos, ValuesUsed used, List<Shake320Hash> matchedHashes) throws VerificationException {
        ...
        if (height == 0 || !parentOfMatch) {
            ...
            Shake320Hash hash = hashes.get(used.hashesUsed++);
    ...
    private static Shake320Hash combineLeftRight(byte[] left, byte[] right) {
        return new Shake320Hash(reverseBytes(singleDigestTwoBuffers(reverseBytes(left), reverseBytes(right))));
    }
     ...
     */
    public Shake320Hash getTxnHashAndMerkleRoot(List<Shake320Hash> matchedHashesOut) throws VerificationException {
        ...
        if (transactionCount > Block.MAX_BLOCK_SIZE / 128) // 128 is the lower bound for the size of a serialized CTransaction
        ...
        Shake320Hash merkleRoot = recursiveExtractHashes(height, 0, used, matchedHashesOut);
