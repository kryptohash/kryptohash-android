...
public class StoredTransactionOutput implements Serializable {
     ...
     * @param scriptBytes
     */
    public StoredTransactionOutput(Shake320Hash hash, long index, Coin value, int height, boolean isCoinbase, byte[] scriptBytes) {
        this.hash = hash;
        this.index = index;
        this.value = value;
    ...
    public StoredTransactionOutput(Shake320Hash hash, TransactionOutput out, int height, boolean isCoinbase) {
        this.hash = hash;
    ...
    public StoredTransactionOutput(InputStream in) throws IOException {
        ...
        if (in.read(hashBytes) != 32)
            throw new EOFException();
        hash = new Shake320Hash(hashBytes);
