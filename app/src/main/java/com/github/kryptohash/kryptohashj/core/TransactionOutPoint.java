...
import com.github.kryptohash.kryptohashj.script.*;
...
public class TransactionOutPoint extends ChildMessage implements Serializable {
    ...
    static final int MESSAGE_LENGTH = 44;

    /** Hash of the transaction to which we refer. */
    private Shake320Hash hash;
    ...
    public TransactionOutPoint(NetworkParameters params, long index, @Nullable Transaction fromTx) {
        super(params);
        this.index = index;
        if (fromTx == null && index == TransactionInput.GENESIS) {
            // This happens when constructing the genesis block.
            hash = Shake320Hash.ZERO_HASH;
        } else if (fromTx != null) {
            this.hash = fromTx.getHash();
            this.fromTx = fromTx;
        } else {
            throw new VerificationException("Cannot create TX Out");
        }
        length = MESSAGE_LENGTH;
    }

    public TransactionOutPoint(NetworkParameters params, long index, Shake320Hash hash) {
        super(params);
        this.index = index;
        this.hash = hash;
        length = MESSAGE_LENGTH;
    ...
    /**
     * Returns the Ed25519Key identified in the connected output, for either pay-to-address scripts or pay-to-key scripts.
     * For P2SH scripts you can use {@link #getConnectedRedeemData(com.github.kryptohash.kryptohashj.wallet.KeyBag)} and then get the
     * key from RedeemData.
     * If the script form cannot be understood, throws ScriptException.
     *
     * @return an Ed25519Key or null if the connected key cannot be found in the wallet.
     */
    @Nullable
    public Ed25519Key getConnectedKey(KeyBag keyBag) throws ScriptException {
