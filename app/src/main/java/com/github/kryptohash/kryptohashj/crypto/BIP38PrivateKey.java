...
public class BIP38PrivateKey extends VersionedChecksummedBytes {
    ...
    public Ed25519Key decrypt(String passphrase) throws BadPassphraseException {
        String normalizedPassphrase = Normalizer.normalize(passphrase, Normalizer.Form.NFC);
        Ed25519Key key = ecMultiply ? decryptEC(normalizedPassphrase) : decryptNoEC(normalizedPassphrase);
        Sha3_256Hash hash = KryptohashUtils.doubleSha3_256(key.toAddress(params).toString().getBytes(Charsets.US_ASCII));
    ...
    private Ed25519Key decryptNoEC(String normalizedPassphrase) {
        try {
            ...
            return Ed25519Key.fromSeed(decrypted);
        } catch (GeneralSecurityException x) {
            throw new RuntimeException(x);
        }
    }

    private Ed25519Key decryptEC(String normalizedPassphrase) {
        try {
            ...
            if (hasLotAndSequence) {
                ...
                passFactorBytes = KryptohashUtils.doubleSha3_256(hashBytes).getBytes();
            }
            BigInteger passFactor = new BigInteger(1, passFactorBytes);
            Ed25519Key k = Ed25519Key.fromSeed(passFactor.toByteArray());
            ...
            BigInteger seedFactor = new BigInteger(1, KryptohashUtils.doubleSha3_256(seed).getBytes());
            checkState(passFactor.signum() >= 0);
            checkState(seedFactor.signum() >= 0);
            //BigInteger priv = passFactor.multiply(seedFactor).mod(Ed25519Key.CURVE_PARAMS.getN());

            return Ed25519Key.fromSeed(seedFactor.toByteArray());
        } catch (GeneralSecurityException x) {
            throw new RuntimeException(x);
        }
    }
