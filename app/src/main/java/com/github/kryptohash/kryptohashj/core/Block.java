...
import static com.github.kryptohash.kryptohashj.core.Coin.FIFTY_COINS;
import static com.github.kryptohash.kryptohashj.core.Utils.singleDigest;
import static com.github.kryptohash.kryptohashj.core.Utils.singleDigestTwoBuffers;
 ...
 * specifically using {@link Peer#getBlock(Shake320Hash)}, or grab one from a downloaded {@link BlockChain}.
 */
public class Block extends Message {
    private static final Logger log = LoggerFactory.getLogger(Block.class);
    private static final long serialVersionUID = 2738848929966035282L;
    ...
    public static final long EASIEST_DIFFICULTY_TARGET = 0x280FFFFFL;
    ...
    private transient Shake320Hash hash;
    ...
    /** Constructs a block object from the Kryptohash wire format. */
     ...
     * @param prevBlockHash Reference to previous block in the chain or {@link Shake320Hash#ZERO_HASH} if genesis.
     ...
     *
     * <p>The half-life is controlled by {@link com.github.kryptohash.kryptohashj.core.NetworkParameters#getSubsidyDecreaseBlockCount()}.
     * </p>
     */
    ...
    private void parseHeader() throws ProtocolException {
        ...
        //hash = new Shake320Hash(Utils.reverseBytes(singleDigest(payload, offset, cursor)));
        hash = getKryptohash(payload, offset, cursor).reverse();
    ...
    void writeHeader(OutputStream stream) throws IOException {
        ...
        Utils.uint32ToByteStreamLE(region, stream);
    ...
    private Shake320Hash getKryptohash(byte[] data) {
        if (version <= 1)
            return KryptohashUtils.KShake320v1(data);
        else
            return KryptohashUtils.KShake320v2(data);
    }

    private Shake320Hash getKryptohash(byte[] data, int offset, int length) {
        byte[] toHash = new byte[length];
        System.arraycopy(data, offset, toHash, 0, length);
        return getKryptohash(toHash);
    }
     ...
     * resulting bytes.
     */
    private Shake320Hash calculateHash() {
        try {
            ByteArrayOutputStream bos = new UnsafeByteArrayOutputStream(HEADER_SIZE);
            writeHeader(bos);
            return new Shake320Hash(getKryptohash(bos.toByteArray()).getBytes()).reverse();
        } catch (IOException e) {
    ...
    /**
     * The number that is one greater than the largest representable SHAKE-320
     * hash.
     */
    static private BigInteger LARGEST_HASH = BigInteger.ONE.shiftLeft(320);
    ...
    private void checkMerkleRoot() throws VerificationException {
        Shake320Hash calculatedRoot = calculateMerkleRoot();
    ...
    private Shake320Hash calculateMerkleRoot() {
        List<byte[]> tree = buildMerkleTree();
        return new Shake320Hash(tree.get(tree.size() - 1));
    }
    ...
    private List<byte[]> buildMerkleTree() {
        ...
        for (int levelSize = transactions.size(); levelSize > 1; levelSize = (levelSize + 1) / 2) {
            ...
            for (int left = 0; left < levelSize; left += 2) {
                ...
                tree.add(Utils.reverseBytes(singleDigestTwoBuffers(leftBytes, rightBytes)));
            }
     ...
     */
    public Shake320Hash getMerkleRoot() {
        maybeParseHeader();
        if (merkleRoot == null) {
    ...
    void setMerkleRoot(Shake320Hash value) {
        unCacheHeader();
        merkleRoot = value;
        hash = null;
    }
    ...
    /** Adds the Genesis transaction to this block. */
    public void addGenesis(Transaction t) {
     ...
     */
    public Shake320Hash getPrevBlockHash() {
        maybeParseHeader();
        return prevBlockHash;
    }

    void setPrevBlockHash(Shake320Hash prevBlockHash) {
        unCacheHeader();
        this.prevBlockHash = prevBlockHash;
        this.hash = null;
    }
    ...
    public void setTime(long _time) {
        unCacheHeader();
        this.txTime = _time * 1000;
        this.hash = null;
    }

    public void setTxTime(long _time) {
        this.txTime = _time;
    }

    public void setnTime(long _time) {
        this.time = _time;
    }
     ...
     * {@link com.github.kryptohash.kryptohashj.core.Block#getDifficultyTargetAsInteger()}. Note that this is <b>not</b> the same as
    ...
    void addCoinbaseTransaction(byte[] pubKeyTo, Coin value) {
        ...
        coinbase.addOutput(new TransactionOutput(params, coinbase, value,
                ScriptBuilder.createOutputScript(Ed25519Key.fromPublicOnly(pubKeyTo)).getProgram()));
    ...
    private static final byte[] pubkeyForTesting = new Ed25519Key().getPubKey();
    ...
    Block createNextBlock(@Nullable Address to, @Nullable TransactionOutPoint prevOut, long time,
        ...
        if (to != null) {
            ...
            if (prevOut == null) {
                ...
                byte[] counter = new byte[40];
                counter[0] = (byte) txCounter;
                counter[1] = (byte) (txCounter++ >> 8);
                input.getOutpoint().setHash(new Shake320Hash(counter));
            } else {
    ...
    /**
     * Create a block sending 50KHC as a coinbase transaction to the public key specified.
