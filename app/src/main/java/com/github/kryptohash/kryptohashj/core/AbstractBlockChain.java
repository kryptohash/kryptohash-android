...
import com.github.aelstad.keccakj.kryptohash.SHAKE320;
import com.github.kryptohash.kryptohashj.store.BlockStore;
import com.github.kryptohash.kryptohashj.store.BlockStoreException;
 ...
 * the lightweight SPV mode, a {@link com.github.kryptohash.kryptohashj.store.SPVBlockStore} is the right choice.</p>
 *
 ...
 * {@link com.github.kryptohash.kryptohashj.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
    ...
    class OrphanBlock {
        final Block block;
        final List<Shake320Hash> filteredTxHashes;
        final Map<Shake320Hash, Transaction> filteredTxn;
        OrphanBlock(Block block, @Nullable List<Shake320Hash> filteredTxHashes, @Nullable Map<Shake320Hash, Transaction> filteredTxn) {
    ...
    private final LinkedHashMap<Shake320Hash, OrphanBlock> orphanBlocks = new LinkedHashMap<Shake320Hash, OrphanBlock>();
     ...
     */
    protected abstract StoredBlock getStoredBlockInCurrentScope(Shake320Hash hash) throws BlockStoreException;
     ...
     */
    public Set<Shake320Hash> drainOrphanBlocks() {
        lock.lock();
        try {
            Set<Shake320Hash> hashes = new HashSet<Shake320Hash>(orphanBlocks.keySet());
            orphanBlocks.clear();
            return hashes;
        } finally {
            lock.unlock();
    ...
    private void connectBlock(final Block block, StoredBlock storedPrev, boolean expensiveChecks,
                              @Nullable final List<Shake320Hash> filteredTxHashList,
                              @Nullable final Map<Shake320Hash, Transaction> filteredTxn) throws BlockStoreException, VerificationException, PrunedException {
    ...
    private void informListenersForNewBlock(final Block block, final NewBlockType newBlockType,
                                            @Nullable final List<Shake320Hash> filteredTxHashList,
                                            @Nullable final Map<Shake320Hash, Transaction> filteredTxn,
    ...
    private static void informListenerForNewTransactions(Block block, NewBlockType newBlockType,
                                                         @Nullable List<Shake320Hash> filteredTxHashList,
                                                         @Nullable Map<Shake320Hash, Transaction> filteredTxn,
                                                         StoredBlock newStoredBlock, boolean first,
                                                         BlockChainListener listener,
                                                         Set<Shake320Hash> falsePositives) throws VerificationException {
        if (block.transactions != null) {
    ...
    private void checkDifficultyTransitions(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        ...
        if (newTarget.compareTo(params.getMaxTarget()) > 0)
            throw new VerificationException("Difficulty below proof of work limit");

        BigInteger oldTarget = Utils.decodeCompactBits(prev.getDifficultyTarget());
        int res = newTarget.compareTo(oldTarget);
        if (res > 0) {
            // Difficulty has dropped
            long maxDiffDrop = calcMaxDiffDrop(prev.getDifficultyTarget());
            BigInteger maxRetarget = Utils.decodeCompactBits(maxDiffDrop);
            if (newTarget.compareTo(maxRetarget) > 0)
                throw new VerificationException("Drop in difficulty too large at height " + storedPrev.getHeight() +
                        ": " + Long.toHexString(nextBlock.getDifficultyTarget()) + " vs " + Long.toHexString(maxDiffDrop));
        }
        if (res < 0) {
            // Difficulty has increased
            long maxDiffIncrease = calcMaxDiffIncrease(prev.getDifficultyTarget());
            BigInteger minRetarget = Utils.decodeCompactBits(maxDiffIncrease);
            if (newTarget.compareTo(minRetarget) < 0)
                throw new VerificationException("Increase in difficulty too large at height " + storedPrev.getHeight() +
                        ": " + Long.toHexString(nextBlock.getDifficultyTarget()) + " vs " + Long.toHexString(maxDiffIncrease));
        }
        return;
    }

    private long calcMaxDiffDrop(long bits) {
        long nExp = (int)(bits & 0xFF000000) >>> 24;
        long nbits = (int)(bits & 0x7FFFFF);
        long nDelta = params.getTargetTimespan() * 256;
    ...
    private long calcMaxDiffIncrease(long bits) {
        long nExp = (int)(bits & 0xFF000000) >>> 24;
        long nbits = (int)(bits & 0x7FFFFF);
        long nDelta = params.getTargetTimespan() * 256;
        ...
        if (nbits - nDelta - 0x8000 <= nbits) {
            ...
            long value = nDelta - nbits + 0x7fff;
    ...
    private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev, Block next) throws VerificationException, BlockStoreException {
        ...
        if (timeDelta >= 0 && timeDelta <= NetworkParameters.TARGET_SPACING * 2) {
            ...
            while (!cursor.getHeader().equals(params.getGenesisBlock()) && cursor.getHeight() % params.getInterval() != 0 && cursor.getHeader().getDifficultyTargetAsInteger().equals(params.getMaxTarget()))
                cursor = cursor.getPrev(blockStore);
            BigInteger cursorTarget = cursor.getHeader().getDifficultyTargetAsInteger();
            BigInteger newTarget = next.getDifficultyTargetAsInteger();
            if (!cursorTarget.equals(newTarget))
                throw new VerificationException("Testnet block transition that is not allowed: " +
                    Long.toHexString(cursor.getHeader().getDifficultyTarget()) + " vs " + Long.toHexString(next.getDifficultyTarget()));
        }
    }
    */
     ...
     */
    @Nullable
    public Block getOrphanRoot(Shake320Hash from) {
        lock.lock();
        try {
    ...
    public boolean isOrphan(Shake320Hash block) {
        lock.lock();
        try {
