...
import com.github.kryptohash.kryptohashj.wallet.AllowUnconfirmedCoinSelector;
 ...
 * You must then call {@link PaymentChannelClientState#storeChannelInWallet(Sha3_256Hash)} to store the refund transaction
...
public class PaymentChannelClientState {
    ...
    private final Ed25519Key myKey, serverMultisigKey;
     ...
     *              It is suggested you use at least {@link 1 CENT} to avoid paying fees if you need to spend the refund transaction
     ...
     */
    public PaymentChannelClientState(Wallet wallet, Ed25519Key myKey, Ed25519Key serverMultisigKey,
     ...
     * overriding {@link PaymentChannelClientState#editContractSendRequest(com.github.kryptohash.kryptohashj.core.Wallet.SendRequest)}.
    ...
    public synchronized void initiate() throws ValueOutOfRangeException, InsufficientMoneyException {
        ...
        List<Ed25519Key> keys = Lists.newArrayList(myKey, serverMultisigKey);
    ...
    public synchronized void provideRefundSignature(byte[] theirSignature) throws VerificationException {
        ...
        TxEd25519Signature theirSig = TxEd25519Signature.decodeFromKryptohash(theirSignature);
        ...
        TxEd25519Signature ourSignature =
    ...
    public static class IncrementedPayment {
        public TxEd25519Signature signature;
        public Coin amount;
    }
    ...
    public synchronized IncrementedPayment incrementPaymentBy(Coin size) throws ValueOutOfRangeException {
        ...
        TxEd25519Signature sig = tx.calculateSignature(0, myKey, multisigScript, mode, true);
     ...
     * later.
     *
     * @see PaymentChannelClientState#storeChannelInWallet(Sha3_256Hash)
     */
    ...
    @VisibleForTesting synchronized void doStoreChannelInWallet(Sha3_256Hash id) {
        StoredPaymentChannelClientStates channels = (StoredPaymentChannelClientStates)
                wallet.getExtensions().get(StoredPaymentChannelClientStates.EXTENSION_ID);
     ...
     *           unique.
     */
    public synchronized void storeChannelInWallet(Sha3_256Hash id) {
