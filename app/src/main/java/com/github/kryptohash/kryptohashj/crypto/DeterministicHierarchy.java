 ...
 * ability to hand out {@link DeterministicEd25519Key}s to other people who can then create new addresses
...
public class DeterministicHierarchy implements Serializable {
    private final Map<ImmutableList<ChildNumber>, DeterministicEd25519Key> keys = Maps.newHashMap();
     ...
     */
    public DeterministicHierarchy(DeterministicEd25519Key rootKey) {
        putKey(rootKey);
     ...
     * inserted in order.
     */
    public void putKey(DeterministicEd25519Key key) {
        ...
        final DeterministicEd25519Key parent = key.getParent();
        if (parent != null)
     ...
     */
    public DeterministicEd25519Key get(List<ChildNumber> path, boolean relativePath, boolean create) {
        ...
        if (!keys.containsKey(absolutePath)) {
            ...
            DeterministicEd25519Key parent = get(absolutePath.subList(0, absolutePath.size() - 1), false, true);
     ...
     */
    public DeterministicEd25519Key deriveNextChild(ImmutableList<ChildNumber> parentPath, boolean relative, boolean createParent, boolean privateDerivation) {
        DeterministicEd25519Key parent = get(parentPath, relative, createParent);
        int nAttempts = 0;
     ...
     * @throws IllegalArgumentException if the parent doesn't exist and createParent is false.
     */
    public DeterministicEd25519Key deriveChild(List<ChildNumber> parentPath, boolean relative, boolean createParent, ChildNumber createChildNumber) {
        return deriveChild(get(parentPath, relative, createParent), createChildNumber);
    }

    private DeterministicEd25519Key deriveChild(DeterministicEd25519Key parent, ChildNumber createChildNumber) {
        DeterministicEd25519Key childKey = HDKeyDerivation.deriveChildKey(parent, createChildNumber);
        putKey(childKey);
        return childKey;
    }
     ...
     */
    public DeterministicEd25519Key getRootKey() {
