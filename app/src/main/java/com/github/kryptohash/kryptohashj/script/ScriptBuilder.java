...
//import com.github.kryptohash.kryptohashj.crypto.TransactionSignature;
 ...
 * convenience methods on {@link com.github.kryptohash.kryptohashj.core.Transaction}, but they are useful when working with the
...
public class ScriptBuilder {
    ...
    public static Script createOutputScript(Ed25519Key key) {
     ...
     */
    public static Script createInputScript(@Nullable TxEd25519Signature signature, Ed25519Key pubKey) {
        byte[] pubkeyBytes = pubKey.getPubKeyEncoded();
        byte[] sigBytes = signature != null ? signature.encodeToKryptohash() : new byte[]{};
     ...
     * If given signature is null, incomplete scriptSig will be created with OP_0 instead of signature
     */
    public static Script createInputScript(@Nullable TxEd25519Signature signature) {
        byte[] sigBytes = signature != null ? signature.encodeToKryptohash() : new byte[]{};
        return new ScriptBuilder().data(sigBytes).build();
    }

    /** Creates a program that requires at least N of the given keys to sign, using OP_CHECKMULTISIG. */
    public static Script createMultiSigOutputScript(int threshold, List<Ed25519Key> pubkeys) {
    ...
    public static Script createMultiSigInputScript(List<TxEd25519Signature> signatures) {
        List<byte[]> sigs = new ArrayList<byte[]>(signatures.size());
        for (TxEd25519Signature signature : signatures) {
            sigs.add(signature.encodeToKryptohash());
        }
    ...
    /** Create a program that satisfies an OP_CHECKMULTISIG program. */
    public static Script createMultiSigInputScript(TxEd25519Signature... signatures) {
     ...
     */
    public static Script createP2SHMultiSigInputScript(@Nullable List<TxEd25519Signature> signatures,
     ...
     */
    public static Script createP2SHOutputScript(int threshold, List<Ed25519Key> pubkeys) {
     ...
     * redeem script in the lexicographical sorting order.
     */
    public static Script createRedeemScript(int threshold, List<Ed25519Key> pubkeys) {
        pubkeys = new ArrayList<Ed25519Key>(pubkeys);
        Collections.sort(pubkeys, Ed25519Key.PUBKEY_COMPARATOR);
