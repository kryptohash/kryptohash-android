...
import com.github.kryptohash.kryptohashj.protocols.channels.PaymentChannelCloseException.CloseReason;
...
public class PaymentChannelClient implements IPaymentChannelClient {
    ...
    private final Sha3_256Hash serverId;
    ...
    private final Ed25519Key myKey;
     ...
     *             the server)
     */
    public PaymentChannelClient(Wallet wallet, Ed25519Key myKey, Coin maxValue, Sha3_256Hash serverId, ClientConnection conn) {
     ...
     *                   See {@link com.github.kryptohash.kryptohashj.protocols.channels.IPaymentChannelClient.ClientConnection#acceptExpireTime(long)}
     * @param conn A callback listener which represents the connection to the server (forwards messages we generate to
     *             the server)
     */
    public PaymentChannelClient(Wallet wallet, Ed25519Key myKey, Coin maxValue, Sha3_256Hash serverId, long timeWindow, ClientConnection conn) {
    ...
    private CloseReason receiveInitiate(Protos.Initiate initiate, Coin contractValue, Protos.Error.Builder errorBuilder) throws VerificationException, InsufficientMoneyException {
        ...
        if (!Ed25519Key.isPubKeyCanonical(pubKeyBytes))
            throw new VerificationException("Server gave us a non-canonical public key, protocol error.");
        state = new PaymentChannelClientState(wallet, myKey, Ed25519Key.fromPublicOnly(pubKeyBytes), contractValue, expireTime);
        try {
            state.initiate();
    ...
    private void receiveRefund(Protos.TwoWayChannelMessage refundMsg) throws VerificationException {
        ...
        try {
            ...
            initialMsg.setSignature(ByteString.copyFrom(payment.signature.encodeToKryptohash()));
     ...
     * {@link ClientConnection#destroyConnection(com.github.kryptohash.kryptohashj.protocols.channels.PaymentChannelCloseException.CloseReason)} or
