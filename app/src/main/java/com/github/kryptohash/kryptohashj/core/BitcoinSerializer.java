...
public class BitcoinSerializer {
    ...
    public void serialize(String name, byte[] message, OutputStream out) throws IOException {
        byte[] header = new byte[4 + 4 + 4 + 8 + COMMAND_LEN + 4 + 4];
        ...
        for (int i = 0; i < name.length() && i < COMMAND_LEN; i++) {
            header[20 + i] = (byte) (name.codePointAt(i) & 0xFF);
        }

        uint32ToByteArrayLE(message.length, header, 4 + 4 + 4 + 8 + COMMAND_LEN);

        byte[] hash = KryptohashUtils.doubleSha3_256(message).getBytes();
        System.arraycopy(hash, 0, header, 4 + 4 + 4 + 8 + COMMAND_LEN + 4, 4);
        out.write(header);
        out.write(message);
    ...
    public Message deserialize(ByteBuffer in) throws ProtocolException, IOException {
        // A Kryptohash protocol message has the following format.
        //
        //   - 4 byte magic number: 0xf1110907 for the testnet or
        //                          0xf1ebb49d for production
        //   - 4 version
        //   - 4 region
        //   - 8 #coin
        ...
        //   - Payload data
        //
        // The checksum is the first 4 bytes of a double SHA3_256 hash of the message payload. It isn't
    ...
    public Message deserializePayload(BitcoinPacketHeader header, ByteBuffer in) throws ProtocolException, BufferUnderflowException {
        ...
        // Verify the checksum.
        byte[] cSum = KryptohashUtils.doubleSha3_256(payloadBytes).getBytes();
        if (header.checksum[0] != cSum[0] || header.checksum[1] != cSum[1] || header.checksum[2] != cSum[2] || header.checksum[3] != cSum[3]) {
            throw new ProtocolException("Checksum failed to verify, actual " + HEX.encode(cSum) + " vs " + HEX.encode(header.checksum));
        }
        ...
        byte[] hash = KryptohashUtils.Shake320(payloadBytes).getBytes();
        try {
    ...
    private Message makeMessage(String command, int length, byte[] payloadBytes, byte[] hash, byte[] checksum) throws ProtocolException {
        ...
        if (command.equals("version")) {
            ...
            if (hash != null)
                tx.setHash(new Shake320Hash(Utils.reverseBytes(hash)));
            message = tx;
    ...
    public static class BitcoinPacketHeader {
        /** The largest number of bytes that a header can represent after packet magic */
        public static final int HEADER_LENGTH = 4 + 4 + 8 + COMMAND_LEN + 4 + 4;
        ...
        public BitcoinPacketHeader(ByteBuffer in) throws ProtocolException, BufferUnderflowException {
            ...
            int cursor = 0;
            version = (int) readUint32(header, cursor);
            cursor += 4;
            ...
            for (; header[cursor] != 0 && cursor < COMMAND_LEN + cmdOffset; cursor++);
            if (cursor == cmdOffset)
                throw new ProtocolException("Invalid command");

            byte[] commandBytes = new byte[cursor - cmdOffset];
            System.arraycopy(header, cmdOffset, commandBytes, 0, cursor - cmdOffset);
            try {
