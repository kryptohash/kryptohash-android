 ...
 * <p>-
 * Concrete implementations have to implement {@link #getSignature(com.github.kryptohash.kryptohashj.core.Sha3_256Hash, java.util.List)}
...
public abstract class CustomTransactionSigner extends StatelessTransactionSigner {
    ...
    public boolean signInputs(ProposedTransaction propTx, KeyBag keyBag) {
        ...
        for (int i = 0; i < numInputs; i++) {
            ...
            Sha3_256Hash sighash = tx.hashForSignature(i, redeemData.redeemScript, Transaction.SigHash.ALL, false);
            SignatureAndKey sigKey = getSignature(sighash, propTx.keyPaths.get(scriptPubKey));
            TxEd25519Signature txSig = new TxEd25519Signature(sigKey.sig, Transaction.SigHash.ALL, false);
            int sigIndex = inputScript.getSigInsertionIndex(sighash, sigKey.pubKey);
            inputScript = scriptPubKey.getScriptSigWithSignature(inputScript, txSig.encodeToKryptohash(), sigIndex);
            txIn.setScriptSig(inputScript);
        }
        return true;
    }

    protected abstract SignatureAndKey getSignature(Sha3_256Hash sighash, List<ChildNumber> derivationPath);

    public class SignatureAndKey {
        public final Ed25519Key.stdEd25519Sig sig;
        public final Ed25519Key pubKey;

        public SignatureAndKey(Ed25519Key.stdEd25519Sig sig, Ed25519Key pubKey) {
            this.sig = sig;
            this.pubKey = pubKey;
        }
    }
